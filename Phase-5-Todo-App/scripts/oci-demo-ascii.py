...
def demo_oci_authentication_check():
    ...
    try:
        ...
        if config_file.exists():
            print(f"[OK] Found OCI config file: {config_file}")
            # Normally we would load with: config = oci.config.from_file()
        else:
            print(f"[WARN] OCI config file not found at {config_file}")
...
def demo_oci_services_connection():
    ...
    if Path(vault_config_path).exists():
        print(f"[OK] Found vault configuration in {vault_config_path}")
