...
class TodoAgent:
    ...
    def __init__(self):
        ...
        EXTRACTING PRIORITY AND DUE DATES:
        - If user mentions priority words like "urgent", "important", "critical" → set priority to "high"
        - If user mentions "low priority", "not urgent", "whenever" → set priority to "low"
        - Default priority is "medium" if not specified
        - If user mentions dates like "tomorrow", "next week", "Monday", "by Friday" → extract and set due_date
        - Date formats: "2026-02-10", "tomorrow", "next Monday", "in 3 days"
        - If user says "high priority task to buy milk tomorrow" → extract both priority="high" and due_date

        Examples:
        - "Add urgent task to call doctor" → priority="high"
        - "Remind me to buy groceries tomorrow" → due_date="2026-02-09" (tomorrow's date)
        - "Low priority task to clean room" → priority="low"
        - "Important meeting on Monday" → priority="high", due_date="2026-02-10"
    ...
    def _get_tool_definitions(self):
        ...
        return [
            {
                "name": "add_task",
                "description": "Add a new task for the user. Can include priority (low/medium/high) and due date.",
                "parameter_definitions": {
                    ...
                    "priority": {
                        "type": "string",
                        "description": "Task priority: 'low', 'medium', or 'high'. Extract from user's message - words like 'urgent', 'important' mean 'high', 'low priority' means 'low'. Default is 'medium'.",
                        "required": False
                    },
                    "due_date": {
                        "type": "string",
                        "description": "Due date in ISO format YYYY-MM-DD. Extract from phrases like 'tomorrow', 'next Monday', 'by Friday', 'on 2026-02-10'. Leave empty if no date mentioned.",
                        "required": False
                    }
                }
            },
